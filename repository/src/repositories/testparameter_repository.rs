...
impl IRepository<TestParameter> for TestParameterRepository {
    ...
    ) -> anyhow::Result<Vec<TestParameter>> {
        let mut client = Connection::fn_repo_get_connection_sqlsever().await?;
        let mut rows = client
            .query(
                "SELECT * FROM testparameter WHERE setting_version_id =@P1",
                &[&setting_version_id],
            )
            .await?
            .into_first_result()
    ...
    async fn fn_repo_get_by_id_postgresql(id: i32) -> anyhow::Result<TestParameter> {
        // TODO: Implement PostgreSQL get by id logic
        Err(anyhow::anyhow!("Not implemented"))
    }

    async fn fn_repo_get_all_postgresql() -> anyhow::Result<Vec<TestParameter>> {
        // TODO: Implement PostgreSQL get all logic
        Err(anyhow::anyhow!("Not implemented"))
    }

    async fn fn_repo_create_postgresql(obj: TestParameter) -> anyhow::Result<TestParameter> {
        // TODO: Implement PostgreSQL create logic
        Err(anyhow::anyhow!("Not implemented"))
    }

    async fn fn_repo_update_postgresql(obj: TestParameter) -> anyhow::Result<TestParameter> {
