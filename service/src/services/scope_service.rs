...
use crate::iservices::iscope_service::IScopeService;
...
impl IScopeService for ScopeService {
    async fn fn_ser_get_by_id(id: i32) -> anyhow::Result<ScopeView> {
        ...
        let data = match dbtype.as_str() {
            ...
            "PostgreSql" => ScopeRepository::fn_repo_get_by_id_postgresql(id).await?,
    ...
    async fn fn_ser_get_all() -> anyhow::Result<Vec<ScopeView>> {
        ...
        let data = match dbtype.as_str() {
            ...
            "PostgreSql" => ScopeRepository::fn_repo_get_all_postgresql().await?,
    ...
    async fn fn_ser_create(obj: ScopeView) -> anyhow::Result<ScopeView> {
        ...
        let result = match dbtype.as_str() {
            ...
            "PostgreSql" => ScopeRepository::fn_repo_create_postgresql(data).await?,
    ...
    async fn fn_ser_update(obj: ScopeView) -> anyhow::Result<ScopeView> {
        ...
        let result = match dbtype.as_str() {
            ...
            "PostgreSql" => ScopeRepository::fn_repo_update_postgresql(data).await?,
    ...
    async fn fn_ser_delete(id: i32) -> anyhow::Result<bool> {
        ...
        let result = match dbtype.as_str() {
            ...
            "PostgreSql" => ScopeRepository::fn_repo_delete_postgresql(id).await?,
    ...
    ) -> anyhow::Result<ScopeView> {
        let dbtype = CONFIGS.app_setting.dbtype.clone();
        let data = match dbtype.as_str() {
            "SQLServer" => {
                ScopeRepository::fn_repo_get_by_setting_version_sqlserver(setting_version_id)
                    .await?
            }
            "PostgreSql" => {
                ScopeRepository::fn_repo_get_by_setting_version_postgresql(setting_version_id)
                    .await?
            }
            _ => ScopeRepository::fn_repo_get_by_id_sqlserver(setting_version_id).await?,
        };
        let json = serde_json::to_string(&data)?;
        let result = serde_json::from_str(&json)?;
        Ok(result)
